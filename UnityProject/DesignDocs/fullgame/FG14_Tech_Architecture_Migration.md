...
2. **远征时家园遭袭**需要整个世界同时运行（家园、前哨站、远征队、行进中的突袭），Demo 出发时会退出家园。
...
- 家园地图是一张按种子生成的**无限格网**（FG17），按区块存储（**初值：32 × 32 格**），1 格 = 1 米。阵营领地与家园在同一张格网地图上；室内场地是独立的小格网表面（FGR-ARC-014）。
...
### FGR-ARC-002 整个世界同时运行
- 世界是一张连续的星球地图（FG17），另有少量室内表面。**模拟**与**镜头**分离：
  - 模拟：星球上所有活跃区块（有己方实体、行进中的突袭和巡逻、正在被观察）都在运行；远处未被惊动的敌方据点休眠（FGR-GEN-052、FGR-ARC-016）；室内表面有己方单位时与星球同时运行。
  - 镜头：在星球上自由平移和连续缩放；只渲染镜头附近的区块，其余区块不生成表现对象。
- 改造要点：
  - Demo 的家园、破碎都市、铸造前哨三个区域控制器，合并为同一个世界模拟；`CameraDirector` 和 `InputRouter` 由全局的镜头与输入管理器持有。
  - 全局单例按需改为按表面划分的上下文。
  - Demo 的出发事务不再 `Exit()` 家园，改为"派遣"：远征队从家园出发，沿地形行进（FG08）。
- 必须满足 FGR-BASE-021：未被观察的区域，模拟结果和被观察时一致。

### FGR-ARC-003 战斗逐单位逻辑下沉内核
- 正式版规模（见 FG15）下，逐单位、逐弹体的循环不允许留在热更层（项目架构第 4 条）。
- **默认方向**：把远征和家园突袭的战斗接入 `SimWorld` 内核，复用它已有的弹道（`JobProjectile`）和基元读法。热更层只下发命令、读取汇总结果。
...
- 新增状态域：格网、传送带和物品、管线和流体、研究、天气与时间、突袭、事件、任务、常驻规则、信号核、掉落数据包、统计、**世界种子与生成器版本、各表面的区块差异**（只保存被修改过的区块，FGR-GEN-060）。
...
### FGR-ARC-011 测试基础设施
- 扩展 ER8 的旅程机器人框架，支持 FG 里程碑的旅程；旅程机器人在固定测试种子上运行，不依赖固定坐标。
- 物流和战斗内核要有不依赖场景的无头测试。
...
- 生成器改动必须提升版本号；旧存档用旧版本生成未修改的区块（FGR-GEN-061）。

### FGR-ARC-014 表面
- 表面分两种：星球（连续、无限，v1 只有地球）和室内（从入口进入的有限小地图）（FGR-GEN-010、011）。家园、各阵营领地、白潮滩头都在同一个星球表面上。
- 每个表面独立存储区块、独立模拟；镜头进出室内时切换表面，这是星球上唯一不无缝的地方。
- 存档格式必须能在不破坏旧存档的前提下新增星球（候选里程碑 FG-X1）。

### FGR-ARC-015 层级寻路
- 无缝大地图上的长距离移动（远征队行军、突袭行进）用层级寻路：先在区块级的连通图上规划路线，再在局部用流场或 A* 细化。
- 区块连通图随地形变化（建筑、屏障、净化）增量更新。
- 长距离寻路在工作线程计算，不阻塞主线程；同一目标的多个单位共享路线。

### FGR-ARC-016 休眠与唤醒
- 远处的敌方据点与巡逻按 FGR-GEN-052 休眠，不参与每帧模拟。
- 唤醒按确定性规则补算休眠期间应有的变化（例如增援计时），保证与"一直在模拟"的结果一致。
...
| Story | 验证内容 | 通过标准 |
|---|---|---|
| FG0-ARCH-01 | 整个世界同时运行；统一游戏时钟底座 | 家园、一支远征队、一次行进中的突袭同时模拟 30 分钟游戏时间；镜头在它们之间来回飞跃 100 次；未被观察的部分与被观察时结果逐字段一致；统一时钟驱动一切，暂停和倍速同步 |
...
| FG0-ARCH-06 | 层级寻路与休眠唤醒 | 2,000 格路线 ≤ 20 ms（工作线程）；同目标单位共享路线；休眠据点唤醒后与持续模拟的对照一致 |
